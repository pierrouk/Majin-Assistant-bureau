...
void NetworkManager::fetchWeather() {
    // 1. Si désactivé dans les réglages -> On nettoie l'affichage
    if (!_settings->getWeatherEnabled()) {
        Serial.println("🚫 [Network]: Météo désactivée -> Stop Effets");
        if (_weatherCallback) _weatherCallback(-1); // Code d'arrêt
        return;
    }
...
void NetworkManager::setupWebServer() {
    ...
    _server.on("/", HTTP_GET, [this](AsyncWebServerRequest *request){
        request->send(200, "text/html", PageDashboard::getHTML());
    });
    ...
    _server.on("/settings", HTTP_GET, [this](AsyncWebServerRequest *request){
        request->send(200, "text/html", PageSettings::getHTML());
    });

    // 4. Personnalisation (Météo + Affichage)
   // Route Perso Mise à jour
    _server.on("/perso", HTTP_GET, [this](AsyncWebServerRequest *request){
        ...
        int mood = _settings->getManualMood(); // ⬅️ Récupération
        
        request->send(200, "text/html", PagePersonalization::getHTML(t, s, lat, lon, en, boot, mood));
    });
    ...
    // 6. Tamagotchi
    _server.on("/tamagotchi", HTTP_GET, [this](AsyncWebServerRequest *request){
        request->send(200, "text/html", PageTamagotchi::getHTML());
    });

    // 7. Deck Config (Multi-pages)
    ...
    _server.on("/api/config/weather", HTTP_POST, [this](AsyncWebServerRequest *request){
        if (request->hasParam("lat")) {
            ...
            _settings->setWeatherEnabled(request->getParam("enable")->value() == "1");
...
// ⬅️ API POUR FORCER L'HUMEUR
    _server.on("/api/config/mood", HTTP_POST, [this](AsyncWebServerRequest *request){
        if (request->hasParam("val")) {
            int val = request->getParam("val")->value().toInt();
            _settings->setManualMood(val);
            
            // Petit hack : Si on repasse en Auto (-1), on force un recalcul immédiat
            if (val == -1) _core->wakeUp(); // Réveille le calcul
    ...
    _server.on("/api/countdown/delete", HTTP_POST, [this](AsyncWebServerRequest *request){
        _settings->setEventName("");
        _settings->setEventTimestamp(0);
    ...
    // Actions Système
    _server.on("/api/wifi/reset", HTTP_POST, [this](AsyncWebServerRequest *request){
        request->send(200); delay(500); resetWiFi(); ESP.restart();
    });
    _server.on("/api/reset", HTTP_POST, [this](AsyncWebServerRequest *request){
        request->send(200); delay(500); resetWiFi(); _settings->factoryReset(); ESP.restart();
    });
...
String NetworkManager::getIP() {
    if (_connected) return WiFi.localIP().toString();
    return "0.0.0.0";
}
